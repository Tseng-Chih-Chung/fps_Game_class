...
public class PlayerCam : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        xRotation = Mathf.Clamp(xRotation, -75f, 45f); // 限定X軸轉動在正45度到負75度間(抬頭和低頭有限制角度)

        transform.rotation = Quaternion.Euler(xRotation, yRotaiton, 0); // 更新攝影機角度至滑鼠移動變量

        RaycastHit hit;
        if (Physics.Raycast(transform.position, transform.forward, out hit))
        {
            DebugPoint.position = hit.point;
        }
    }
}
